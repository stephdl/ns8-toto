...
# Reuse existing nodebuilder-toto container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-toto; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-toto -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
